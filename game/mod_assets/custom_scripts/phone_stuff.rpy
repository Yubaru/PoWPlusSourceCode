...
default phone_conversation_labels = {}

init python:
    
    def register_phone_conversation(group_chat_key, conversation_label):
        """
        Registers a conversation label for a group chat.
        
        Args:
            group_chat_key: The key of the group chat
            conversation_label: The label name that contains the phone discussion
        """
        phone_conversation_labels[group_chat_key] = conversation_label
    
    def send_phone_message(group_chat_key, sender, message, conversation_label=None):
        """
        Sends a phone message and triggers notification/vibration.
        
        Args:
            group_chat_key: The key of the group chat to send the message to
            sender: The character sending the message
            message: The message text
            conversation_label: Optional label to trigger when phone is clicked. 
                               If None, uses registered label or opens contacts.
        """
        # Register the message (this will mark it as unread)
        phone.discussion.register_message(group_chat_key, sender, message)
        
        # Register conversation label if provided
        if conversation_label:
            register_phone_conversation(group_chat_key, conversation_label)
        
        # Play vibration sound if available
        if renpy.loadable("mod_assets/sfx/phone_message.ogg"):
            renpy.play("mod_assets/sfx/phone_message.ogg", channel="sound")
        
        # Force screen update to show notification
        renpy.restart_interaction()
    
    def get_phone_conversation_label(group_chat_key):
        """
        Gets the conversation label for a group chat.
        
        Args:
            group_chat_key: The key of the group chat
            
        Returns:
            The conversation label name, or None if not registered
        """
        return phone_conversation_labels.get(group_chat_key)