...
init python:
    ...
    # the game launches.
    splash_message_default = "One last time... before we say goodbye."
...
# Uses Christmas splash image when Christmas mode is enabled
image intro:
    truecenter
    "white"
    0.5
    ConditionSwitch(
        "persistent.christmas_mode", "bg/splash_jolly.png",
        "True", "bg/splash.png"
...
label splashscreen:
    ...
    # Use the main menu music from mainmenu.rpy, or Christmas music if Christmas mode is enabled
    if persistent.christmas_mode:
        $ config.main_menu_music = christmas_music
        $ renpy.music.play(christmas_music, fadein=3.0)
        # Set Christmas message
        $ splash_message = "Merry Christmas!"
    else:
        $ config.main_menu_music = mainmenu_music
        $ renpy.music.play(config.main_menu_music, fadein=3.0)
        # Use default message or random message in Act 2
...
## This label sets the main menu music before the menu starts.
## Uses the custom mainmenu_music variable from mainmenu.rpy
## Plays Christmas music if Christmas mode is enabled
label before_main_menu:
    if persistent.christmas_mode:
        $ config.main_menu_music = christmas_music
        # Ensure the music plays when returning to main menu
        $ renpy.music.play(christmas_music, fadein=1.0, if_changed=True)
    else:
        $ config.main_menu_music = mainmenu_music
        # Ensure the music plays when returning to main menu
        $ renpy.music.play(mainmenu_music, fadein=1.0, if_changed=True)
    return
